...
class BizHawkEnv(gym.Env):
	...
	def __init__(self, logging_folder_path=None, algorithm_name="DQN", state_representation="SS", reward_representation="DISTANCE", state_frame_count=1, no_action=False, human_warm_up_episode=0, active_debug_text=True, replay=False):
	...
	def step(self, action):
		...
		if self.active_debug_text and self.curr_step % 15 == 0:
			# sys.stdout.write(f"Reward: {self.cumulative_reward:4.2f}   Action Taken: {action_code} in step {self.curr_step}           \r")
			# print(f"Reward: {self.cumulative_reward:4.2f}")
			sys.stdout.flush()
	...
	def reset(self):
		...
		# if self.curr_episode < self.human_warm_up_episode + 1:
		# 	self.no_action = True
		# else:
	...
	def _get_state(self):
		def current_vector_from_ss_with_last_action():
			self.update_current_vector_bizhawk_screenshot()
			combined_state = np.append(self.current_vector, self.last_action)
...
#            RAM_state.append(int.from_bytes(byte_value, byteorder="little", signed=True))
	...
	def start_recording_bizhawk(self):
		...
		pyautogui.moveTo(229, 180, duration=0.15)
		pyautogui.click()
		self.proc.stdin.write(b'emu.frameadvance() ')
		self.proc.stdin.flush()
		pyautogui.moveTo(229, 190, duration=0.15)
		pyautogui.click()
		self.proc.stdin.write(b'emu.frameadvance() ')
		self.proc.stdin.flush()
		pyautogui.moveTo(317, 396, duration=0.15)
		pyautogui.click()
		self.proc.stdin.write(b'emu.frameadvance() ')
		self.proc.stdin.flush()
		pyautogui.moveTo(504, 450, duration=0.15)
		pyautogui.click()
		self.proc.stdin.write(b'emu.frameadvance() ')
		self.proc.stdin.flush()
		pyautogui.moveTo(430, 431, duration=0.15)
		pyautogui.click()
		self.proc.stdin.write(b'emu.frameadvance() ')
		self.proc.stdin.flush()
		pyautogui.moveTo(417, 463, duration=0.15)
		pyautogui.click()
		self.proc.stdin.write(b'emu.frameadvance() ')
		self.proc.stdin.flush()
		pyautogui.moveTo(601, 495, duration=0.15)
		pyautogui.click()
		self.proc.stdin.write(b'emu.frameadvance() ')
		self.proc.stdin.flush()
		pyautogui.moveTo(1022, 606, duration=0.15)
